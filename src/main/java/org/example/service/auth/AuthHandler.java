...
public class AuthHandler
{
    ...
    public AuthHandler()
    {
        this.jwtAuth = JWTAuth.create(Main.getVertx(), new JWTAuthOptions().addPubSecKey(new PubSecKeyOptions()
                        .setAlgorithm(MotaDataConfigUtil.getConfig()
                                .getString(JWT_ENCRYPTION_ALGORITHM, Constants.JWT_ENCRYPTION_ALGORITHM))
                        .setBuffer(MotaDataConfigUtil.getConfig()
                                .getString(JWT_SECRET_KEY, Constants.JWT_SECRET_KEY))));
    }

    private String generateAccessToken(JsonObject claims)
    {
        return jwtAuth.generateToken(claims, new JWTOptions().setExpiresInSeconds(MotaDataConfigUtil.getConfig()
                .getInteger(JWT_EXPIRY_TIME_IN_SECONDS , Constants.JWT_EXPIRY_TIME_IN_SECONDS))); // 60 minutes
    }

    private String generateRefreshToken(JsonObject claims)
    {
        return jwtAuth.generateToken(claims, new JWTOptions().setExpiresInSeconds(JWT_EXPIRY_REFRESH_TIME_IN_SECONDS)); // 7 days
    }
                    ...
                    if (USERNAME.equals(body.getString(USERNAME)) && PASSWORD.equals(body.getString(PASSWORD)))
                    {
                        ...
                        context.response()
                                ...
                                .addCookie(Cookie.cookie(REFRESH_TOKEN, generateRefreshToken(claims))
                                        ...
                                        .setMaxAge(JWT_EXPIRY_REFRESH_TIME_IN_SECONDS))  // 7 days
                                .end(new JsonObject()
        ...
        try
        {
            ...
            jwtAuth.authenticate(new TokenCredentials(cookie.getValue()))
                    ...
                    .onFailure(err ->
                            context.response()
                                    .setStatusCode(Constants.SC_401)
                                    .end(new JsonObject().put(Constants.SUCCESS, Constants.FALSE).put(Constants.MESSAGE, MESSAGE_INVALID_TOKEN).encode())
                    );
        }
        catch (Exception exception)
        {
